...
namespace Hard_012
{
    ...
    public static class Program
    {
        ...
        {
            var n = ReadValue<int>();
            var dList = ReadList<int>().ToArray();
            ...
            while (dicl.Last() == 0)
            {
                dicl.RemoveAt(dicl.Count - 1);
            }

            // d[0]が根にならないといけないので、根が複数あるのはありえない。
            if (dicl[0] > 1)
            {
                Console.WriteLine(0);
                return;
            ...
            var sum = new ModInt(dicl[0]);

            for (var i = 1; i < dicl.Count(); i++)
            {
                var before = dicl[i - 1];
                var current = dicl[i];

                var modInt = new ModInt(before).Power(current);
                sum *= modInt;
            }
