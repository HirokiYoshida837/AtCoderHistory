...
namespace ABC177C
{
    public static class Program
    {
        public static void Main(string[] args)
        {
            ...
            var aList = Console.ReadLine().Split().Select(long.Parse).Select(x => new ModInt(x)).ToArray();

            var cum = new List<ModInt> {aList[0]};
            for (int i = 1; i < n; i++)
            {
                cum.Add(cum[^1] + aList[i]);
            }

            var sum = new ModInt(0);
            for (int i = 0; i < n; i++)
            {
                var right = cum.Last() - cum[i];
                sum += aList[i] * right;
            }
